/*
 * libsmc fork for The Onion Box
 * Copyright (C) Ralph Wetzel
 * License MIT
 * www.theonionbox.com
 *
...
*/
static kern_return_t call_smc_conn(io_connect_t con,
                                   ...
                                   SMCParamStruct *outputStruct)
{
    ...
    result = IOConnectCallStructMethod(con, kSMCHandleYPCEvent,
                                            inputStruct,
                                            inputStructCnt,
                                            outputStruct,
                                            &outputStructCnt);
...
static kern_return_t call_smc(SMCParamStruct *inputStruct,
                              SMCParamStruct *outputStruct)
{
    return call_smc_conn(conn, inputStruct, outputStruct);
}
...
*/
static kern_return_t read_smc_conn(io_connect_t con, char *key, smc_return_t *result_smc)
{
    kern_return_t result;
    SMCParamStruct inputStruct;
    SMCParamStruct outputStruct;
    ...
    result = call_smc_conn(con, &inputStruct, &outputStruct);
...
{
    return read_smc_conn(conn, key, result_smc);
}
...
*/
static kern_return_t write_smc_conn(io_connect_t con, char *key, smc_return_t *result_smc)
{
    kern_return_t result;
    SMCParamStruct inputStruct;
    SMCParamStruct outputStruct;
...
{
    return write_smc_conn(conn, key, result_smc);
}
...
kern_return_t open_smc_conn(io_connect_t *con)
{
    kern_return_t result;
    io_service_t service;
    ...
    result = IOServiceOpen(service, mach_task_self(), 0, con);
    IOObjectRelease(service);
...
{
    return open_smc_conn(&conn);
}


kern_return_t close_smc_conn(io_connect_t con)
{
...
{
    return close_smc_conn(conn);
}


bool is_key_valid_conn(io_connect_t con, char *key)
{
    bool ans = false;
    kern_return_t result;
    ...
    result = read_smc_conn(con, key, &result_smc);
...
{
    return is_key_valid_conn(conn, key);
}


double get_tmp_conn(io_connect_t con, char *key, tmp_unit_t unit)
{
    kern_return_t result;
    smc_return_t  result_smc;
...
{
    return get_tmp_conn(conn, key, unit);
}
